...
// Register fonts used by the PDF
Font.register({
  family: "Helvetica",
  fonts: [
    {
...
export default function InvoiceDocument({
  ...
  if (
    ...
    cgst = Number(
      ((subtotal * gstRate) / 200).toFixed(2)
    );
  ...
  const numberToWords = (amount: number) => {
    ...
    const convertIndianNumber = (num: number) => {
      ...
      if (crore > 0) {
        parts.push(
          `${convertIndianNumber(crore)} Crore`
        );
      }

      if (lakh > 0) {
        parts.push(
          `${convertBelowHundred(lakh)} Lakh`
        );
      }

      if (thousand > 0) {
        parts.push(
          `${convertBelowHundred(thousand)} Thousand`
        );
      }
    ...
    const rupeesText = convertIndianNumber(
      integerPart
    );

    return `${rupeesText} Rupees${
      decimalPart > 0
        ? ` and ${decimalPart} Paise`
        : ""
    } Only`;
  };
  ...
  const upiUri = firm?.upi_id
    ? `upi://pay?pa=${encodeURIComponent(
        String(firm.upi_id)
      )}&pn=${encodeURIComponent(
        String(firm.firm_name || "Finetax")
      )}&am=${balanceDue.toFixed(
        2
            ...
            >
                  ...
                  >
                    ...
                    >
                      {invoice?.notes
                        ...
                        : "Thank you for your business. Please make payment as per the agreed terms."}
                    </Text>
                  </View>
              ...
              <View style={styles.footer}>
                <View style={styles.footerLeft}>
                  ...
                  >
                    This is a
                    computer-generated tax invoice and
